...
def show_callchain(nid, data):
    ...
    if nid not in nodes:
        ...
        return
    adj = {}
    call_edges = {}  # (from,to) -> list[edge]（支持同一 from-to 多条边）
    for e in edges:
        if e["rel"] == "CALLS":
            adj.setdefault(e["from"], []).append(e["to"])
            call_edges.setdefault((e["from"], e["to"]), []).append(e)
    ...
    def dfs(node, parent, prefix, is_last, depth):
        ...
        if parent:
            edge_list = call_edges.get((parent, node), [])
            at_locs = [ploc(nodes.get(parent, {}).get("path"), e.get("at_line", "?")) for e in edge_list]
            at_str = " / ".join(at_locs) if at_locs else "?"
        else:
            at_str = "-"
        print(f"{prefix}{branch}{node}  path= {ppath(n)}  ← 调用点: {at_str}")
...
def show_callers(nid, data):
    ...
    if nid not in nodes:
        ...
        return
    rev_adj = {}
    call_edges = {}  # (to, from) -> list[edge]（反向记录，支持同一 from-to 多条边）
    for e in edges:
        if e["rel"] == "CALLS":
            rev_adj.setdefault(e["to"], []).append(e["from"])
            call_edges.setdefault((e["to"], e["from"]), []).append(e)
    ...
    # 去重：rev_adj 的调用者列表中同一节点可能因多条边出现多次
    callers = rev_adj.get(nid, [])
    seen = {nid}
    unique_callers = [c for c in callers if c not in seen and not seen.add(c)]
    if not unique_callers:
        print("  （无人调用它——没有 CALLS 入边）")
        return

    # 初始层：每个 (caller, at_line) 单独一行
    for i, c in enumerate(unique_callers):
        edge_list = call_edges.get((nid, c), [])
        n_c = nodes.get(c, {})
        for j, e in enumerate(edge_list):
            last = (i == len(unique_callers) - 1 and j == len(edge_list) - 1)
            branch = "└── " if last else "├── "
            at_loc = ploc(n_c.get("path"), e.get("at_line", "?"))
            print(f"  {branch}{c}  path= {ppath(n_c)}  ← 调用点: {at_loc}")
            if j == 0:
                print(f"  {'    ' if last else '│   '}    {n_c.get('desc', '')}")

    # 递归展开子树（跳过初始层的边，只展开 parents）
    # 预填充 visited：初始层节点不重新进入，避免递归打印它们的入边时又连回自己
    visited = {nid}
    for c in unique_callers:
        visited.add(c)

    def dfs_expand(node, prefix, is_last, depth):
        if depth > 8:
            print(f"{prefix}{'└── ' if is_last else '├── '}…（超过 8 层，截断）")
            return
        n = nodes.get(node, {})
        parents = [p for p in rev_adj.get(node, []) if p not in visited and not visited.add(p)]
        for i_p, p in enumerate(parents):
            last_p = (i_p == len(parents) - 1)
            n_p = nodes.get(p, {})
            p_edge_list = call_edges.get((node, p), [])
            for j_p, e_p in enumerate(p_edge_list):
                last_e = (i_p == len(parents) - 1 and j_p == len(p_edge_list) - 1)
                branch = "└── " if last_e else "├── "
                at_loc = ploc(n_p.get("path"), e_p.get("at_line", "?"))
                print(f"{prefix}{branch}{p}  path= {ppath(n_p)}  ← 调用点: {at_loc}")
                if j_p == 0:
                    print(f"{prefix}{'    ' if last_e else '│   '}    {n_p.get('desc', '')}")
            nxt = prefix + ("    " if last_p else "│   ")
            dfs_expand(p, nxt, last_p, depth + 1)

    for i, c in enumerate(unique_callers):
        dfs_expand(c, "  ", i == len(unique_callers) - 1, 1)
        ...
        if p:
            ...
            if not full.exists():
                ...
                if m:
                    ...
                    if kind in ("function", "gitCommit") and is_python and not re.search(r"def |class |async def ", target):
